...
int fmincg(void(*costFunc)(COST_FUNC_DATATYPE* inputVector, COST_FUNC_DATATYPE* cost, COST_FUNC_DATATYPE* gradVector), COST_FUNC_DATATYPE* xVector, int maxCostCalls,int dim)
{
	...
	COST_FUNC_DATATYPE *df1, *s, *x0, *df0, *df2, *tmp;
	df1 = new float[dim];
	s = new float[dim];
	x0 = new float[dim];
	df0 = new float[dim];
	...
	if (costFuncCount >= maxCostCalls) {
		delete df1; delete s; delete x0; delete df0; delete df2; delete tmp;
		return 1;
	}
	else
		costFuncCount++;
